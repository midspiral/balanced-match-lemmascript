...
- **Body-balance as a transferred theorem about `range_spec`** — the body-balance equality (`CountAll(str, a, r[0]+1, r[1]+|a|-1) == CountAll(str, b, …)`) is no longer a separate loop invariant; the universal refinement (`res == range_spec(a, b, str)`) means any property of the spec function transfers automatically to `range`. Stating and proving body-balance as `lemma SpecBodyBalance(a, b, str) requires NoOverlap(a, b, str) ensures range_spec(...) == Some(v) ==> …` is now a self-contained theorem about a pure function — no entanglement with the imperative loop's verification budget. (Earlier attempts to prove body-balance directly as a loop invariant hit Z3 resource walls: per-stack-frame `forall j :: CountAll(str, …, begs[j]+1, …) == pushedCount - pushedAtPush[j]` requires quantifier instantiation × recursive `CountAll` unfolding × `--isolate-assertions`, which combinatorially exceeds the verifier's budget. The refinement approach sidesteps this entirely by keeping the loop's invariant a single function equation instead of a quantified counting claim.)
...
The diff between `index.dfy.gen` and `index.dfy` is additions-only — the spec functions (`range_spec`, `range_spec_loop`), proof-only lemmas (`CountAll`, `IndexOfMaxAt`, `CountAllExtend*`, `CountAllExtendsByOne`, `CountAllShrinkLo`, `CountAllZeroFromOtherStart`, `NoOverlapAtAPos`, `BranchTwoBodyBalance`), loop invariants Dafny needs that can't be expressed in the TS surface, plus bridging asserts that thread facts from invariants to use-sites. The method-level `ensures res == range_spec(a, b, str)` is the headline universal claim.
