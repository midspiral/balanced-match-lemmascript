...
| **Body-balance for branch-2 results (Phase 4)** | When `range_spec` returns a pair `[s, e]` via the branch-2 fully-closed path (i.e., the same pair that `range_spec_inner` — a no-fallback variant of the spec — returns), the interior is Dyck-balanced: `CountAll(str, a, s+1, e+|a|-1) == CountAll(str, b, s+1, e+|b|-1)`. Proved by `SpecBodyBalance(a, b, str)` as a pure-function theorem: induction on `range_spec_loop_inner`'s recursive structure, with per-frame CountAll accounting threaded via ghost parameters (`b0`, `aiLowerBound`, `biLowerBound`); branch 2 reduces to `BranchTwoBodyBalance`. Fallback-derived pairs are not covered — see "What's not (yet) verified." |
...
## Phase 4 proof structure (body-balance as a derived theorem)

`SpecBodyBalance(a, b, str)` — under `NoOverlap` and `a != b`, when the algorithm closes a matched pair via branch 2, the inner-substring `CountAll(a, …)` equals the inner-substring `CountAll(b, …)`. Proved entirely against the pure spec function — no entanglement with the imperative method's verification. The proof rests on four pieces:
...
- **Body-balance for fallback-derived results** — the body-balance theorem `SpecBodyBalance` (Phase 4) covers branch-2-derived results, where the algorithm fully closes a matched pair via the `|begs| == 1` pop. The complementary case — when the loop exits with `right.Some(r)` and `|begs| > 0`, returning `[left, r]` via the post-loop fallback — is not yet verified. It would require an additional invariant tracking `[left, r]`'s body-balance across the branch-3 `popped < left` updates: at each update the new `[popped, bi]` pair needs body-balance shown from the per-frame `CountAll(…, begs[j]+1, …)` accounting, which is the same shape of `forall j` claim that hit Z3 resource walls in the original Phase 2 imperative attempt. The Phase 4 approach (proof as a self-contained lemma about a pure spec function, opaque `BodyBalancedOpt` predicate to keep `CountAll` out of the ensures-level VC) may make this tractable; left as future work.
...
```
Dafny program verifier finished with 587 verified, 0 errors
```
...
The diff between `index.dfy.gen` and `index.dfy` is additions-only — the spec functions (`range_spec`, `range_spec_loop`, plus the no-fallback variants `range_spec_inner` and `range_spec_loop_inner` for Phase 4), proof-only lemmas (`CountAll`, `IndexOfMaxAt`, `CountAllExtend*`, `CountAllExtendsByOne`, `CountAllShrinkLo`, `CountAllZeroFromOtherStart`, `NoOverlapAtAPos`, `NoOverlapPred` / `NoOverlapInstance`, `BranchTwoBodyBalance`, `BodyBalancedOpt`, `SpecBodyBalanceLoopInner`, `SpecBodyBalanceInner`, `RangeSpecLoopInnerAgreement`, `SpecBodyBalance`), loop invariants Dafny needs that can't be expressed in the TS surface, plus bridging asserts that thread facts from invariants to use-sites. The method-level `ensures res == range_spec(a, b, str)` is the headline universal claim; `SpecBodyBalance` is the Phase 4 derived theorem.
